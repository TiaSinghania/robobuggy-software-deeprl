...
class ExperimentModelDynamics:
    def __init__(
        self, env_name="Pushing2D-v1", num_nets=1, mpc_params=None, device=None
    ):
        self.env = gym.make(env_name)
        self.task_horizon = TASK_HORIZON
        ...
        self.model = PENN(
            num_nets, STATE_DIM, len(self.env.action_space.sample()), LR, device=device
        )
        self.cem_policy = MPC(
            self.env,
            PLAN_HORIZON,
    ...
    def test(self, num_episodes, optimizer="cem"):
        ...
        for j in range(num_episodes):
            if j == 0 or (j + 1) % INFO == 0:
                log.info("Test episode {}".format(j))
            samples.append(
                self.agent.sample(
                    self.task_horizon,
                    self.cem_policy if optimizer == "cem" else self.random_policy,
                )
            )
    ...
    def train(self, num_train_itrs, num_episodes_per_itr, evaluation_interval):
        ...
        for i in range(num_train_itrs):
            ...
            log.info(
                "Rewards obtained: {}, Rollout Length: {}".format(
...
def test_cem_gt_dynamics(num_episode=50):
    ...
    log.info(
        "CEM PushingEnv: avg_reward: {}, avg_success: {}".format(
    ...
    log.info(
        "Random PushingEnv: avg_reward: {}, avg_success: {}".format(
            avg_reward, avg_success
        )
    )
    ...
    avg_reward, avg_success = exp.test(num_episode, optimizer="random")
    log.info(
        "Random + MPC PushingEnv: avg_reward: {}, avg_success: {}".format(
            avg_reward, avg_success
        )
    )
    ...
    for env_name in ["Pushing2D-v1", "Pushing2DNoisyControl-v1"]:
        ...
        avg_reward, avg_success = exp.test(num_episode, optimizer="cem")
        log.info(
            "CEM {}: avg_reward: {}, avg_success: {}".format(
                env_name[:-3], avg_reward, avg_success
            )
        )
        ...
        avg_reward, avg_success = exp.test(num_episode, optimizer="cem")
        log.info(
            "CEM + MPC {}: avg_reward: {}, avg_success: {}".format(
                env_name[:-3], avg_reward, avg_success
            )
        )
...
def train_single_dynamics(num_test_episode=50, device=None):
    log.info(
        "### Q1.2.1: Train a single dynamics model f(s, a, phi) using a random policy"
    )
    num_nets = 1
    num_episodes = 1000
    num_train_itrs = 100
    mpc_params = {"use_mpc": True, "num_particles": 6}
    exp = ExperimentModelDynamics(
        env_name="Pushing2D-v1", num_nets=num_nets, mpc_params=mpc_params, device=device
    )

    log.info(
        "### Q1.2.1: Train from 1000 randomly sampled episodes with 100 iterations"
    )
    ...
    log.info(
        "Single + Random: avg_reward: {}, avg_success: {}".format(
            avg_reward, avg_success
        )
    )
    ...
    log.info(
        "Single + CEM: avg_reward: {}, avg_success: {}".format(avg_reward, avg_success)
    )
...
def train_pets(device=None):
    ...
    log.info(
        "### Q1.3.1: Train from 100 randomly sampled episodes + 500 MPC iterations"
    )
    ...
    for i, name in enumerate(["Returns", "Success Rate"]):
        plt.figure(dpi=100)
        plt.plot(
            np.arange(len(cme_test)) * 50, np.array(cme_test)[:, i], label="CEM + MPC"
        )
        plt.plot(
            np.arange(len(cme_test)) * 50,
            np.array(rnd_test)[:, i],
            label="Random + MPC",
        )
        ...
        plt.legend()
        plt.savefig(
            "out/%d-cem-vs-random-%s.png" % (num_nets, name), bbox_inches="tight"
        )
...
if __name__ == "__main__":
    gpu_number = 0
    device = torch.device(
        "cuda:%d" % gpu_number if torch.cuda.is_available() else "cpu"
    )
